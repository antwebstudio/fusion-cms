...
class SettingServiceProvider extends ServiceProvider
{
    ...
    {
        $this->settings = Cache::rememberForever('fusion-settings', function() {
    ...
    {
        $config = Cache::rememberForever('fusion-setting-config', function() {
            $config = [];
            SettingGroup::all()->each(function ($group) use(&$config) {
                if ($group->fieldset) {
                    $group->fieldset->fields->each(function ($field) use ($group, &$config) {
